...
def test_create_menu():
    ...
    response = client.post(app.url_path_for('get_menus_url'), json=MY_MENU_1)
...
def test_get_none_submenu():
    # Получение указанного подменю
    response = client.get(app.url_path_for('get_submenu_url', menu_id=MENU_ID, submenu_id=0))
...
def test_update_submenu():
    ...
    response = client.patch(app.url_path_for('update_submenu_url', menu_id=MENU_ID,
                                             submenu_id=SUBMENU_ID), json=MY_SUBMENU_2)
    assert response.status_code == 200
    assert response.json().get('id') == SUBMENU_ID
...
def test_update_none_submenu():
    ...
    #                         json=MY_SUBMENU_2)
    response = client.patch(app.url_path_for('update_submenu_url', menu_id=MENU_ID,
                                             submenu_id=0), json=MY_SUBMENU_2)
    assert response.status_code == 404
    assert response.json() == {'detail': 'submenu not found'}
...
def test_delete_submenu():
    ...
    response = client.delete(app.url_path_for('delete_submenu_url', menu_id=MENU_ID,
...
def test_delete_none_submenu():
    ...
    response = client.delete(app.url_path_for('delete_submenu_url', menu_id=MENU_ID, submenu_id=0))
    assert response.status_code == 404
